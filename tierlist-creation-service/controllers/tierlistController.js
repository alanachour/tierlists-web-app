...
export const getTierLists = async (req, res) => {
  try {
    ...
    res.json(tierLists);  // Sends all TierLists as JSON
  } catch (error) {
...
export const createTierList = async (req, res) => {
  try {
    ...
    if (!title || !coverImage || !categories) {
      return res.status(400).json({ message: 'Champs requis manquants.' });  // Validates required fields
    }
    ...
    res.status(201).json(savedTierList);  // Returns the newly created TierList
  } catch (error) {
